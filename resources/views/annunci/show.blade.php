...
<div class="container">
    ...
    @php
        ...
        $conditionTranslation = $conditions[$annuncio->condition] ?? $annuncio->condition;
        $adTypeTranslation = $adTypes[$annuncio->adType] ?? $annuncio->adType;
    @endphp
    <p class="card-text display-5">Condizione: {{ $conditionTranslation }}</p>
    <p class="card-text display-5">Tipo di annuncio: {{ $adTypeTranslation }}</p>
