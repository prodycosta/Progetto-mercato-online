...
<div class="container">
    ...
    @foreach($chats as $chat)
        <div class="card mb-3 mx-auto">
            <a href="{{ route('chat.index', ['receiver' => $chat->receiver->id, 'annuncio' => $chat->article_id ?? 0]) }}">
                <div class="card-container d-flex align-items-center">

                    @if($chat->article && $chat->article->fotos->isNotEmpty())
                        <div class="card-left">
                            @foreach ($chat->article->fotos as $foto)
                    ...
                    <div class="card-body">
                        <h5 class="card-title">{{ $chat->receiver->name }}</h5>
                        <p class="card-subtitle mb-2 text-muted">{{ optional($chat->article)->title }}</p>
                        <p class="card-text">- {{ Str::limit($chat->lastMessage->message, 25) }}</p>
                        <p class="card-text">{{ $chat->last_message_time }}</p>
                    </div>
                </div>
            </a>
        </div>
